...
ctk.set_appearance_mode("System")  # Appearance: 'System', 'Light' or 'Dark'
ctk.set_default_color_theme("blue")  # Color theme: 'blue', 'green' or 'dark-blue'
...
def copy_file(source, target, buffer_size=1024 * 1024):
    ...
    try:
        ...
        raise IOError(f"Error copying file from {source} to {target}: {e}")
...
class SyncHandler(FileSystemEventHandler):
            ...
            for root, dirs, files in os.walk(self.source_folder):
                ...
                for file in files:
                    ...
                    if os.path.exists(target_file) and os.path.getsize(source_file) == os.path.getsize(target_file):
                        continue  # Skip if the file sizes are the same

                    try:
                        copy_file(source_file, target_file)
                        self.log_callback(f"Copied: {source_file} -> {target_file}")
                    except Exception as e:
                        self.log_callback(f"Error copying file: {e}")
    ...
    def backup_deleted_file(self, file_path):
        ...
        try:
            ...
            self.log_callback(f"Error backing up file: {e}")
...
class App(ctk.CTk):
    ...
    def __init__(self):
        ...
        self.queue = queue.Queue()  # Queue for inter-thread communication
    ...
    def log_callback(self, message):
        ...
        self.queue.put(message)  # Add message to queue
    ...
    def process_queue(self):
        ...
        self.after(100, self.process_queue)  # Update UI every 100 ms
